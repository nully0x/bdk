...
fn test_walk_ancestors() {
    ...
    let ancestors = [
        graph
            .walk_ancestors(Arc::new(tx_c0.clone()), |depth, tx| Some((depth, tx)))
            .collect::<Vec<_>>(),
        graph
            .walk_ancestors(Arc::new(tx_d0.clone()), |depth, tx| Some((depth, tx)))
            .collect::<Vec<_>>(),
        graph
            .walk_ancestors(Arc::new(tx_e0.clone()), |depth, tx| Some((depth, tx)))
            .collect::<Vec<_>>(),
        // Only traverse unconfirmed ancestors of tx_e0 this time
        graph
            .walk_ancestors(Arc::new(tx_e0.clone()), |depth, tx| {
